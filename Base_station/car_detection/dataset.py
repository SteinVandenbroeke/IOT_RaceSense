...
def parse_label_file(label_path: str):
    ...
    with open(label_path, "r") as f:
        for line in f:
            ...
            cls  = int(vals[0])
            # Parse the true format [xmin, ymin, xmax, ymax]
            xmin, ymin, xmax, ymax = vals[1], vals[2], vals[3], vals[4]
            ...
            bottom_kpt_indices = [0, 2, 4, 6]

            for i in bottom_kpt_indices:
                kx_img = kpt_vals[i * 3]
                ky_img = kpt_vals[i * 3 + 1]
                v = kpt_vals[i * 3 + 2]

                # Correct bbox-relative math!
                if true_w > 0 and true_h > 0:
                    kx_rel = (kx_img - xmin) / true_w
                    ky_rel = (ky_img - ymin) / true_h
                else:
                    kx_rel, ky_rel = 0.0, 0.0

                kp_xy.append([kx_rel, ky_rel])
                kp_v.append(1.0 if v > 0 else 0.0)

            # Append the corners, as train.py expects them!
...
def load_sample(img_path: str, lbl_path: str, augment: bool = False):
    ...
    # Augmentation
    if augment:
        img, boxes, kpts, vis = _augment(img, boxes, kpts, vis)

    return img, boxes, kpts, vis


def _augment(img, boxes, kpts, vis):
    """
    Applies consistent augmentation to image + labels.
    Handles bounding boxes in [xmin, ymin, xmax, ymax] format.
    Handles bbox-relative keypoints and their visibility flags.
    """
    # Random horizontal flip
    if tf.random.uniform(()) < AUG_FLIP_PROB:
        img = tf.image.flip_left_right(img)

        # --- 1. CORRECTED BOX FLIP ---
        # Extract the corners (format is xmin, ymin, xmax, ymax)
        xmin = boxes[:, 0:1]
        ymin = boxes[:, 1:2]
        xmax = boxes[:, 2:3]
        ymax = boxes[:, 3:4]

        # Flip the x coordinates and swap min/max
        # (The old right edge becomes the new left edge)
        new_xmin = 1.0 - xmax
        new_xmax = 1.0 - xmin

        # Reassemble the bounding box
        boxes = tf.concat([new_xmin, ymin, new_xmax, ymax], axis=1)

        # --- 2. KEYPOINT COORDINATE FLIP ---
        # Flip the bbox-relative x coordinate inside the box
        kx = 1.0 - kpts[:, :, 0:1]
        kpts = tf.concat([kx, kpts[:, :, 1:2]], axis=2)

        # --- 3. SEMANTIC INDEX SWAPPING ---
        # Swap the left and right keypoints so the model knows which is which.
        indices = tf.constant([1, 0, 3, 2], dtype=tf.int32)

        # Apply the swap to both the coordinates AND the visibility flags
        kpts = tf.gather(kpts, indices, axis=1)
        vis = tf.gather(vis, indices, axis=1)
