...
def custom_ssd_loss(y_true, y_pred, anchors):
    ...
    pred_kpt_flat = tf.reshape(pred_kpt, [tf.shape(pred_kpt)[0], tf.shape(pred_kpt)[1], 16])
    gt_kpt_flat = tf.reshape(matched_kpt_targets,
                             [tf.shape(matched_kpt_targets)[0], tf.shape(matched_kpt_targets)[1], 16])
...
def train():
    ...
    for epoch in range(EPOCHS_PHASE1):
        for batch in train_ds:
            # Pass the anchors into the train step
    ...
    for epoch in range(EPOCHS_PHASE2):
        for batch in train_ds:
            loss = train_step(model, optimizer_p2, batch)
